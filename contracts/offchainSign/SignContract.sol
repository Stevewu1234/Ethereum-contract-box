...
contract SignTestContract is EIP712, Ownable {
    ...
    bytes32 public constant PERMIT_TYPEHASH = keccak256("PermitWithValue(address owner,uint256 value,string testString,uint256 deadline,uint256 nonce)");
    ...
    function testSignatureMessage(
        ...
        string memory _testString,
        uint256 deadline
    ) external view returns (bytes32 domainHash, bytes32 structHash, bytes32 messageHash) {
    ...
    /** ========== event ========== */
    event PermittedWithValue(address signer, uint256 value, string testString, uint256 deadline);
