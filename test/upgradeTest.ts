...
beforeEach(async () => {
  ...
  const proxyTestAdminFactory = await hre.ethers.getContractFactory('ProxyTestAdmin')
  ...
  proxyTest = await proxyTestFactory.deploy(logic1.address, proxyTestAdmin.address, '');
...
describe('initial setting check', () => {
    it('upgradeable contract deploymentg checking', async () => {
        expect(await proxyTestAdmin.getProxyAdmin(proxyTest.address)).to.equal(proxyTestAdmin.address);
        expect(await proxyTestAdmin.getProxyImplementation(proxyTest.address)).to.equal(logic1.address);
    })
})